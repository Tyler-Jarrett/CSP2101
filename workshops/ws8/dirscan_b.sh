...
case $opt in
    f|F) # if full path option is selected
        if ! [[ -d ${HOME}${dir} ]]; then # if the path/dir does not exist, inform user and exit script with error code
            ...
            path=$(echo -n "${HOME}${dir}/*${file_type}") # else assign valid path to variable with wildcard operator
        fi
    ;;
    c|C|s|S) # if cwd or subdirectory path option is selected
        if ! [[ -d ${dir} ]]; then # if the path/dir do not exist, inform user and exit script with error code
            ...
            path=$(echo -n "${dir}/*${file_type}") # else assign valid path to variable with wildcard operator
        fi
    ;;
...
do
    if [[ -f $item ]]; then # if a file
        ...
        let total_size=$total_size+$(du -b $item | cut -f 1)
...
converted_size=$(getsize $total_size)
echo -e "\nThe total size is $converted_size"
